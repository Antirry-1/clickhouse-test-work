-- ============================================================================
-- cohort_lifecycle.sql — когортный анализ и жизненный цикл «клиента» на витрине.
--
--   · стадии жизненного цикла: new → active → inactive → churned;
--   · шаги когортного анализа: когорта → метрика → временная разбивка.
--
-- «Клиент» здесь = получатель (`receiver`, MSISDN абонента): у него достаточно
-- кардинальности для когорт (у самого `customer_id` их всего 8). Запросы читают
-- представление _active. Синтаксис — ClickHouse 24.8.
--
-- ВНИМАНИЕ: данные синтетические — получатели разыгрываются равномерно из пула по
-- стране в течение года, поэтому реального retention-сигнала в них нет. Запросы
-- демонстрируют ТЕХНИКУ, а не реальную динамику продукта.
-- ============================================================================
